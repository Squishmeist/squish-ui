...
function readUiFile(slug: string, filename: string) {
  try {
    return fs.readFileSync(
      path.join(process.cwd(), "components/mobile", slug, filename),
      "utf-8",
    );
  } catch {
...
function readTestResult(
  ...
  try {
    const raw = fs.readFileSync(
      path.join(process.cwd(), "test-results-mobile.json"),
      "utf-8",
    );
...
export function ComponentMobileDoc({
  name,
  slug,
  description,
  previews,
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 sm:px-8 py-8 sm:py-12">
      ...
      <section id="tests">
        ...
        <TestRunner slug={slug} builtResult={builtResult} runner="jest" />
      </section>
